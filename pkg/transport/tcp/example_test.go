...
func ExampleClient() {
	socket, err := tcp.Dial("localhost:5678", rs.NewSetupPayload("text/json", "text/json", nil, nil))
...
func ExampleServer() {
	server, err := tcp.Listen(":0", func(setup rs.ConnectionSetupPayload, socket rs.ReactiveSocket) (*rs.RequestHandler, error) {
